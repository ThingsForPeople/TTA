...
- **Batting order**: 9 slots, pitcher typically bats 9th. The analyzer recommends orders via role-based slot assignment (leadoff=OBP, #2=wOBA+OBP, #4=wOBA+ISO, rest plain wOBA) refined by an **exact Markov expected-runs model** (`expectedLineupRuns` in `analysis.ts`, 2026-07-16 — replaced the Monte-Carlo sim whose noise floor exceeded its own acceptance threshold; deterministic, `MIN_SIM_GAIN` 0.005 is a true indifference band). All profile inputs (wOBA/OBP/ISO/K%/BB%) regress on one PA-confidence ramp toward a replacement anchor (0.85× league) that entered CON/POW attributes shift (up to 1.15×); injuries discount by severity; the run model regresses to the same anchor (a stat-less bat no longer sims as league-average). Chain talents (Rally Time, Clutch Cascade — two batters, per the index) are modeled as state in the run model in both modes; slot-affinity talent values apply in talent mode only. A **platoon variant** ("vs LHP/RHP" in the panel) shifts each bat by its replay-derived split (`platoonDeltas`). Removed as unsupported post-patch: the #5 "protection" −K% tilt and #8 second-leadoff tilt (BB% ~2 — no pitch-arounds; the pitcher-8th pattern measures ~0.000 R/g), steal/baserunning order bonuses (steals are run-negative, 36% CS), and the "pitcher bats 8th" UI narrative. Total ordering stakes ≈ 0.2 R/g end to end
