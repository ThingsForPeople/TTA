...
- `src/lib/talentIndex.json` + `talentIndex.ts` — the **official Talent Index** (scraped from https://www.tiny-teams.com/talents by `scripts/sync-talent-index.ts`; site added it 2026-07-09). The ONLY source of talent effect **magnitudes**: 66 talents, per-tier prose (`prose.range` = "+6/8/10/12% Power per charge", `prose.perTier` 1–4), trigger `tags`, and `synergy` (battery-partner bonuses with their own numbers). Ids are internal engine ids == replay `effect.activated.talentId` == `talents.ts` ids. **Talents cap at Tier 4** (no Tier 5 — old Lv5 references were wrong). Zone (`hz:*`) and pitch-arsenal talents are NOT in the index (generated variants, no published numbers). Categories include `gauntlet` (mode-only talents/afflictions, excluded from `talents.ts`). `system-prompt.ts` merges the magnitudes into the AI talent reference. Re-run the sync script after talent patches
